...
class BasicEpisode(Episode):
    ...
    def step(self, action_as_int, sim=None, viewpoint = None):
        ...
        if action["action"] != DONE:
            ...
            self.done_count += 1
        if sim is not None:
            self.target_sim[sim["target"]] = sim
        self.target_viewpoint = viewpoint
    ...
    def judge(self, action):
        ...
        self.CS_MAX = 0
        reward_sim = self.get_partial_reward_Sim()
        reward_dts = self.distance_reward()
        reward_success = 0
        reward_view = 0


        self.done_dis2goal = max(self.state2goal(self.environment.controller.state, self.task_data) - 1, 0)
        self.FTR = max(self.state2goal(self.environment.controller.state, self.task_data), 0)
        ...
        if action["action"] == DONE:
            ...
            for id_ in self.task_data:
                if self.environment.object_is_visible(id_):
                    reward_success = GOAL_SUCCESS_REWARD
                    reward_view = self.viewpoint_reward()
                    done = True
        ...
        reward = reward_step + reward_sim + reward_dts + reward_success + reward_view
        return reward, done, action_was_successful

    def get_partial_reward_Sim(self):
        reward = 0
        curren_objs = self.environment.current_objs
        for obj in curren_objs.keys():
            if obj in self.objects:
                CS = self.target_sim.get(self.target_object, {}).get(obj, 0)
                if CS > self.CS_MAX:
                    self.CS_MAX = CS
    ...
    def distance_reward(self):
        ...
        dis_reward = (d_last - d_now)*0.1
        return dis_reward

    def viewpoint_reward(self):
        reward = 0
        if self.target_viewpoint is not None:
            reward = -(abs(self.target_viewpoint[0] - 0.5) + abs(self.target_viewpoint[1] - 0.5)) * 0.1
        return reward
